	...
	<div class="studyWrite-container">
		<form id="writeForm" method="post" action="${contextPath}/study/studyModify" enctype="multipart/form-data">
			<table width="1180px" id="studyTable" class="table lineremove">
				...
				<c:forEach begin="0" end="${listSize}" step="1" var="n" >
					<c:forEach items="${scontentList}" var="scontent" varStatus="contentNo">
						<c:if test="${scontent.scontent_order_no==n}">
							<tr class="numChecking">
								<td colspan="3">
									<div class="textCustom">
										<span class="nameTag">색상 : </span><input type="text" name="scontentList[${contentNo.index}].scontent_color" value="${scontent.scontent_color}" class="textCss scontent_color" />
									...
									<div class="buttonsPosition">
										...
										<input type="button" value="X" class="trDelete btn btn-danger btn-sm">
										<input type="hidden" name="scontentList[${contentNo.index}].scontent_order_no" class="order_no scontent_order_no" value="${n}" />
									</div>
								</td>
							</tr> 
						</c:if>
					</c:forEach>
					<c:forEach items="${simgList}" var="simg" varStatus="simgNo">
						<c:if test="${simg.simg_order_no==n}">
							<tr class="numChecking">
								<td colspan="3" class="currentImg">
									<div class="imgCustom" >	
										<span class="nameTag">이미지 너비 : </span><input type="text" name="simgList[${simgNo.index}].simg_width" value="${simg.simg_width}" class="textCss simg_width" />
										<span class="nameTag">이미지 높이 : </span><input type="text" name="simgList[${simgNo.index}].simg_height" value="${simg.simg_height}" class="textCss simg_height" />
									</div>
									<div class="file-input-css">
										<input id="fileName" class="file-input-text" readonly />
										<div class="file-input-div">
											<input type="button" value="File Upload" class="file-input-button btn btn-primary btn-sm">
											<input type="file" name="imgfiles" class="fileUpload modifyFile file-input-hidden">
										</div>   
									</div>
										<input type="button" value="▲" class="positionUp btn btn-secondary btn-sm" >
										<input type="button" value="▼" class="positionDn btn btn-secondary btn-sm">
										<input type="button" value="X" class="trDelete deleteFile btn btn-danger btn-sm">
										<input type="hidden" name="simgList[${simgNo.index}].simg_order_no" class="order_no simg_order_no" value="${simg.simg_order_no}" />
										<input type='hidden' name="reposition" class="reposition${simg.simg_no} fixNum" value="${simg.simg_no}" />
										<input type='hidden' name='repositionValue' class='reposition${simg.simg_no} repositionValue' value="${simg.simg_order_no}"/>
										<input type="hidden" name="simgList[${simgNo.index}].is_upload" value="0" class="is_upload" />
			...
			<div class="submitPosition">
				<input type="submit" name="regist" class="inputBtn btn btn-primary" value="수정하기" />
				<input type="button" name="previous" id ="previous" class="inputBtn btn btn-primary" value="취소" />
			</div>
		</form>
	</div>
<script>
	$(function(){
			...
			$('.modifyFile').change(function(){
				var fixNum =$(this).nextAll('.fixNum').val()
				var modifyFile = "<input type='hidden' class='modifyFile"+fixNum+"' name='modifyFile' value='"+fixNum+"' />"	;
				$(modifyFile).appendTo('#studyTable');
			});
			
			
			//파일 삭제시 파라미터 전달
			$('.deleteFile').click(function(){
				var deleteNum =$(this).nextAll('.fixNum').val()
				var deleteModifyFile = ".modifyFile"+deleteNum;
				$(deleteModifyFile).remove();
				var deleteFile="<input type='hidden' name='deleteFile' value='"+deleteNum+"' />";
				$(deleteFile).appendTo('#studyTable');
			});
